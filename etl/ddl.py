...
BUG-2  : FAIT_REGLEMENTS — primary key column renamed id_regl → id_reglement
           to match the DBML schema declaration.
...
_DDL_GROUPE_1: list[tuple[str, str]] = [
    ...
    # BUG note: semestre, jour_semaine, est_weekend, est_ferie are ETL-computed
    # extensions not in DBML schema but generated by _generate_dim_date().
    ("DIM_DATE", """
...
CREATE TABLE FAIT_LIGNES_VENTE (
    ...
    # BUG-1 : delai_reel_jours and ecart_delai are INT (not SMALLINT) to avoid
    #          truncation for debts older than ~90 years (SMALLINT max = 32 767 days)
    # BUG-2 : primary key renamed id_regl → id_reglement to match DBML schema
    # BUG-3 : RT_NbJour SMALLINT NULL restored — present in DBML schema,
    #          computed from F_REGLEMENTT, was silently dropped without this column
    ("FAIT_REGLEMENTS", """
CREATE TABLE FAIT_REGLEMENTS (
    ...
    # BUG-4 : alerte_tension is an ETL-computed column (not in DBML) that stores
    #          NULL for non-stock rows and 0/1 for stock rows — handled in transform.py
    # BUG-5 : id_fournisseur added (supplier-linked ecritures from pipeline)
    ("FAIT_ECRITURES", """
...
_MIGRATIONS: list[tuple[str, str]] = [
    ...
    # BUG-3: RT_NbJour was missing from DDL — add to existing DBs
    (
    ...
    # BUG-1: Change delai_reel_jours from SMALLINT to INT on existing DBs
    (
    ...
    # BUG-1: Change ecart_delai from SMALLINT to INT on existing DBs
    (
    ...
    # NOTE: Cannot rename an IDENTITY PK column directly in SQL Server.
    # The canonical fix on a fresh install is the corrected DDL above.
    # On existing DBs we add a computed view alias; the column stays id_regl
    # internally but all new DDL uses id_reglement. A full recreate is needed
    # for a clean rename — document this for the team.
    (
        "FAIT_REGLEMENTS.id_reglement_note",
        "-- BUG-2: id_regl renamed to id_reglement in DDL. "
        "Existing databases require DROP/RECREATE to apply the PK rename. "
        "New installs use id_reglement automatically. "
        "SELECT 1 AS migration_noted",
    ),

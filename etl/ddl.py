...
FIXES APPLIED:
 FIX-1 : DIM_DATE        — added trimestre, semestre
 FIX-2 : DIM_SEGMENT     — added cbIndice_code, prix_ttc_flag, libelle_segment
 FIX-3 : DIM_COLLABORATEUR — CO_Fonction_code replaces raw CO_Fonction
 FIX-4 : FAIT_REGLEMENTS — added id_date_echeance, id_type_doc,
                            BR_TotalReglement, BR_Rapproch, LB_MontantReg,
                            RG_Montant
 FIX-5 : FAIT_ECRITURES  — added CG_Num (was silently dropped before)
 FIX-6 : DIM_BANQUE      — added source SMALLINT (ETL provenance tag)

All fixes are covered by _apply_schema_migrations() for existing databases.
"""
...
_DDL_GROUPE_1: list[tuple[str, str]] = [
    ...
    # FIX-1: added trimestre + semestre
    ("DIM_DATE", """
...
CREATE TABLE DIM_SENS_ECRITURE (
    ...
    code_sens          SMALLINT NOT NULL UNIQUE,
...
CREATE TABLE DIM_TYPE_TVA (
    ...
    code_type_tva      SMALLINT NOT NULL UNIQUE,
...
_DDL_GROUPE_2: list[tuple[str, str]] = [
    ...
    # FIX-2: added cbIndice_code (CRC32 surrogate), prix_ttc_flag, libelle_segment
    ("DIM_SEGMENT", """
...
_MIGRATIONS: list[tuple[str, str]] = [
    ...
    (
        "DIM_SEGMENT.prix_ttc_flag",
        "IF COL_LENGTH('DIM_SEGMENT','prix_ttc_flag') IS NULL "
        "ALTER TABLE [DIM_SEGMENT] ADD prix_ttc_flag SMALLINT NOT NULL DEFAULT 0",
    ),
    (
    ...
    # FIX-3
    (
        "DIM_COLLABORATEUR.CO_Fonction_code",
        "IF COL_LENGTH('DIM_COLLABORATEUR','CO_Fonction_code') IS NULL "
        "ALTER TABLE [DIM_COLLABORATEUR] ADD CO_Fonction_code INT NULL",
    ),
    ...
    (
        ...
        "ADD id_date_echeance INT NULL REFERENCES DIM_DATE(id_date)",
    ),
    (
        "FAIT_REGLEMENTS.id_type_doc",
        "IF COL_LENGTH('FAIT_REGLEMENTS','id_type_doc') IS NULL "
        "ALTER TABLE [FAIT_REGLEMENTS] "
        "ADD id_type_doc INT NULL REFERENCES DIM_TYPE_DOC(id_type_doc)",
    ),
    (
...
def create_all_tables(drop_existing: bool = False) -> None:
    ...
    - drop_existing=True : DROP TABLE IF EXISTS avant création
...
def _apply_schema_migrations(conn) -> None:
    """
    Apply ALL additive migrations for new and existing databases.
    Safe to re-run at every pipeline start.
    """
...
def disable_all_fk(conn) -> None:
    """Désactive toutes les FK constraints DW."""
...
def enable_all_fk(conn) -> None:
    """Réactive et vérifie toutes les FK constraints DW."""

...
import os
import sys
# Ajout du répertoire parent (FINMAG) au sys.path pour permettre les imports absolus "from etl..."
...
def extract_fait_reglements_clients(last_run: Optional[datetime] = None) -> pd.DataFrame:
    delta_where = f"AND rc.RT_Date >= :last_run" if last_run is not None else ""
...
if __name__ == "__main__":
    ...
    output_dir = r"C:\Users\marie\Desktop\myProject\FINMAG\etl\TEMP"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"Démarrage de l'extraction de toutes les tables vers {output_dir}")
    
    # Liste de toutes les fonctions d'extraction (qui ne nécessitent pas d'arguments obligatoires)
    extract_funcs = {
        "exercices_fiscaux": extract_exercices_fiscaux,
        "dim_segment": extract_dim_segment,
        "dim_collaborateur": extract_dim_collaborateur,
        "dim_famille": extract_dim_famille,
        "dim_fournisseur": extract_dim_fournisseur,
        "dim_article": extract_dim_article,
        "dim_client_mag": extract_dim_client_mag,
        "dim_client_grt": extract_dim_client_grt,
        "dim_depot": extract_dim_depot,
        "dim_journal": extract_dim_journal,
        "dim_banque_mag": extract_dim_banque_mag,
        "dim_banque_grt": extract_dim_banque_grt,
        "dim_caisse_mag": extract_dim_caisse_mag,
        "fait_lignes_achat": extract_fait_lignes_achat,
        "fait_lignes_vente": extract_fait_lignes_vente,
        "fait_ecriturec": extract_fait_ecriturec,
        "fait_regtaxe": extract_fait_regtaxe,
        "fait_artstock": extract_fait_artstock,
        "reglementt": extract_reglementt,
        "docentete_dates": extract_docentete_dates,
        "fait_reglech": extract_fait_reglech,
        "fait_reglements_clients": extract_fait_reglements_clients,
        "fait_reglements_fournisseurs": extract_fait_reglements_fournisseurs,
        "docregl_grt": extract_docregl_grt,
        "fait_mvtcaisse": extract_fait_mvtcaisse,
        "dim_type_mvt_caisse": extract_dim_type_mvt_caisse,
        "rfm_data": extract_rfm_data,
        "sales_history_365d": extract_sales_history_365d
    }
    ...
    for name, func in extract_funcs.items():
        try:
            ...
            if isinstance(result, pd.DataFrame):
                ...
                # Sauvegarde en CSV (séparateur point-virgule et encodage UTF-8 pour Excel)
                result.to_csv(file_path, index=False, sep=";", encoding="utf-8-sig")
                logger.info(f" => Sauvegardé: {file_path}")
                
            elif isinstance(result, list): # Cas particulier pour extract_exercices_fiscaux
                file_path = os.path.join(output_dir, f"{name}.csv")
                df_list = pd.DataFrame(result, columns=["Date_Debut", "Date_Fin"])
                df_list.to_csv(file_path, index=False, sep=";", encoding="utf-8-sig")
                logger.info(f" => Sauvegardé: {file_path}")
                
            else:
                logger.warning(f"Le format de retour pour {name} n'est pas pris en charge pour l'export CSV : {type(result)}")
                
        except Exception as e:
            logger.error(f"Erreur lors de l'extraction et sauvegarde de {name}: {e}")
            
    # Traitement spécial pour extract_static_dims qui retourne un dictionnaire de DataFrames
    try:
        logger.info("Exécution de static_dims...")
        static_dims = extract_static_dims()
        for dim_name, df in static_dims.items():
            file_path = os.path.join(output_dir, f"static_{dim_name}.csv")
            df.to_csv(file_path, index=False, sep=";", encoding="utf-8-sig")
            logger.info(f" => Sauvegardé: {file_path}")
    except Exception as e:
        logger.error(f"Erreur lors de l'extraction de static_dims: {e}")
        
    logger.info("Extraction de toutes les sources terminée avec succès.")
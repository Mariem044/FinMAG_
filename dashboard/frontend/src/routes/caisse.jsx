...
function CaissePage() {
  ...
  const alignedFlux = useMemo(() => {
    ...
    const offset = base - lastRawCumul;
    return fluxData.map((d) => ({
      ...d,
      cumul: d.cumul + offset,
    }));
  }, [totalEspeces, totalCheques, fluxData]);
  ...
  const pctChangePrev = useMemo(() => {
    const currentBase = totalEspeces + totalCheques;
    if (currentBase <= 0) return 0;
    return Math.round(((previsionJ15 - currentBase) / currentBase) * 100);
  }, [totalEspeces, totalCheques, previsionJ15]);

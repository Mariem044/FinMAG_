...
def check_kpis_for_year(year):
    with DW_ENGINE.connect() as conn:
        # Get latest month for that year
        ...
        print(f"\n================ YEAR {year} (Latest active month: M{latest_month}) ================")
        ...
        print(f"CA avec cout (N): {ca_avec_cout}")
        ...
        print(f"Recovery Rate (N): {rec_n:.4f}%")
        print(f"Recovery Rate (N-1): {rec_n1:.4f}%")
        print(f"Recovery Rate Growth: {rec_n - rec_n1:.4f}%")

for y in [2024, 2025, 2026]:
    try:
        check_kpis_for_year(y)
    except Exception as e:
        print(f"Error checking year {y}: {e}")

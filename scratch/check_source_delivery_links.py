...
with MAG_ENGINE.connect() as conn:
    print("----- O2S_VW_PREPARATION_LIVRAISON COLUMNS -----")
    sql_cols = """
        SELECT COLUMN_NAME, DATA_TYPE
        FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_NAME = 'O2S_VW_PREPARATION_LIVRAISON'
    """
    for r in conn.execute(text(sql_cols)).fetchall():
        print(f"  {r.COLUMN_NAME}: {r.DATA_TYPE}")
        
    print("\n----- O2S_VW_PREPARATION_LIVRAISON DATA SAMPLE -----")
    try:
        sql_data = "SELECT TOP 10 * FROM O2S_VW_PREPARATION_LIVRAISON"

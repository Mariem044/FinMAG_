...
const PAGES = [
  {
    ...
    keywords: ["dashboard", "accueil", "overview", "tableau"],
  },
  {
    label: "CA & Performance Commerciale",
    description: "Chiffre d'affaires, ventes, familles",
    to: "/ventes",
    icon: TrendingUp,
    keywords: ["ventes", "ca", "chiffre affaires", "commercial", "familles"],
  },
  {
    label: "Trésorerie & Flux de Paiement",
    description: "Encaissements, impayés, recouvrement",
    to: "/tresorerie",
    icon: Wallet,
    keywords: ["tresorerie", "paiement", "flux", "encaissement", "impaye"],
  },
  {
    label: "Stocks & Approvisionnement",
    description: "Inventaire, ruptures, alertes stock",
    to: "/produits",
    icon: Boxes,
    keywords: ["stock", "produit", "inventaire", "rupture", "approvisionnement"],
  },
  {
    label: "Analyse Acteurs & Segmentation",
    description: "Clients, fournisseurs, livreurs, RFM",
    to: "/acteurs",
    icon: Users,
    keywords: ["client", "fournisseur", "acteur", "segment", "rfm", "livreur"],
  },
  {
    label: "Fiscalité & Comptabilité",
    description: "Écritures, TVA, anomalies",
    to: "/fiscalite",
    icon: Receipt,
    keywords: ["fiscalite", "comptabilite", "ecriture", "tva", "journal"],
  },
  {
    label: "Gestion de Caisse",
    description: "Soldes, flux journaliers, prévisions",
    to: "/caisse",
    icon: Banknote,
    keywords: ["caisse", "especes", "solde", "flux"],
  },
  {
    label: "Rapprochement Bancaire",
    description: "Bordereaux, agios, remises en banque",
    to: "/banque",
    icon: Landmark,
    keywords: ["banque", "rapprochement", "bordereau", "agio", "remise"],
  },
  {
    label: "Paramètres",
    ...
    to: "/parametres",
    icon: Settings,
    keywords: ["parametres", "settings", "langue", "devise"],
  },
  {
    label: "Centre d'Aide",
    ...
    to: "/aide",
    icon: HelpCircle,
    keywords: ["aide", "help", "faq", "support"],
  },
];
...
function runSearch(q, apiResults = {}) {
  ...
  const pages = PAGES.filter((p) => matches([p.label, p.description, ...p.keywords], q)).slice(
    0,
    CATEGORY_META.pages.max,
  );
  if (pages.length)
    ...
    <div className="relative flex-1 max-w-xl mx-3">
      ...
      >
        ...
        <span className="hidden lg:flex items-center gap-0.5 flex-shrink-0">
          <kbd className="text-[9px] text-text-dim bg-surface-hover border border-border rounded px-1 py-0.5 font-mono">
            Ctrl
          </kbd>
          <kbd className="text-[9px] text-text-dim bg-surface-hover border border-border rounded px-1 py-0.5 font-mono">
            K
      ...
      {showDropdown && (
        <div
          ref={dropdownRef}
          className="absolute left-0 right-0 top-full z-50 rounded-b-xl border border-t-0 border-border bg-popover shadow-2xl shadow-black/30 overflow-hidden"
        >
                  ...
                  return (
                    <div key={cat}>
                      <div className="flex items-center gap-2 px-3 pt-3 pb-1.5">
                        ...
                        <span
                          className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${meta.bg} ${meta.color}`}
                        >
                          {items.length}
                        </span>
                      ...
                      {items.map((item, i) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${isActive ? meta.bg : "bg-surface-hover"}`}
                            >
                              <Icon
                                size={13}
                                className={isActive ? meta.color : "text-text-muted"}
                              />
                            </div>
              ...
              <div className="border-t border-border/60 px-3 py-2 flex items-center justify-between bg-surface/50">
                <div className="flex items-center gap-3 text-[10px] text-text-dim">
                  <span className="flex items-center gap-1">
                    <kbd className="bg-surface-hover border border-border rounded px-1 font-mono">
                      ↑↓
                    </kbd>{" "}
                    naviguer
                  </span>
